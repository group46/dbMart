module.exports = {
  getProductComments: (req, res, next) => {
    ...
    var queries = [
      "SELECT CONCAT(user.first_name, ', ', user.last_name) AS name, DATE_FORMAT(comment.commentdate, '%Y-%m-%d') AS commentdate, comment.commenttxt AS commenttxt FROM product_posts product, comment_authors comment, user WHERE user.uid = comment.uid AND comment.postid = " + req.params.postid,
